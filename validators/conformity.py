    ...
    def validate(self, df, column_name, allowed_values=None, technical_definition=None, rule_code=None, **kwargs):
        ...
        if effective_rule_code == "RCCONF_113.1":
            ...
            try:
                ...
                for row in rows:
                    g = _norm(row.get("account_group_code"))
                    a = _norm(row.get("reconciliation_account"))
                    if g and a:
            ...
            recon_norm = df[column_name].apply(_norm)
            group_norm = df[account_group_col].apply(_norm)
            ...
            for idx in df.index[evaluated_mask]:
                exists_mask.loc[idx] = (group_norm.loc[idx], recon_norm.loc[idx]) in allowed_pairs
        ...
        # Специальная обработка для правил RCCONF_38.3 и RCCONF_39.3
        # Правило: "TEL_NUMBER Valid characters are numbers: 0123456789 (only when PERSNUMBER is empty)"
        # technical_definition: "IF TEL_NUMBER IS NULL OR TEL_NUMBER = '' THEN '' ELSE IF TEL_NUMBER REGEXP '[^0-9]' THEN '0' ELSE '1'"
        # Важно: пустые значения (NULL) НЕ являются ошибками для этого правила
        # КРИТИЧНО: проверяем только записи с R3_USER = '1' (стационарные телефоны)
        if technical_definition and rule_code in ["RCCONF_38.3", "RCCONF_39.3", "RCCONF_39.3.2"]:
            # RCCONF_39.3.2: данные уже отфильтрованы по заполненному PERSNUMBER в checker — только проверка цифр
            if rule_code == "RCCONF_39.3.2":
                df_filtered = df.copy()
            else:
                # RCCONF_38.3, RCCONF_39.3: фильтруем только R3_USER = '1' (стационарные телефоны)
                r3_user_col = None
                ...
                if r3_user_col:
                    ...
                    if not base_mask.any():
                        return 0, 0, None  # нет строк с R3_USER='1' — 0 оценённых
            ...
            if non_null_mask.any():
                # Проверяем, что значение содержит только цифры (REGEXP '[^0-9]' означает наличие не-цифр)
                for idx in df_filtered[non_null_mask].index:
                    tel_value = str(df_filtered.loc[idx, column_name]).strip()
                    
                    # Пропускаем пустые значения (они уже обработаны выше, но на всякий случай)
                    if not tel_value or tel_value.lower() in ['none', 'null', 'nan', '']:
                        continue
                    
                    # Проверяем, что только цифры (REGEXP '[^0-9]' означает наличие не-цифр = ошибка)
                    if re.search(r'[^0-9]', tel_value):
                        error_mask.loc[idx] = True  # Ошибка: есть не-цифры
            
            error_count = error_mask.sum()
            if rule_code == "RCCONF_39.3.2":
                error_description = f'Invalid telephone number format in {column_name}. Must contain only digits (0-9). Empty values are allowed. (Only rows with filled PERSNUMBER.)'
            else:
                error_description = f'Invalid telephone number format in {column_name}. Must contain only digits (0-9). Empty values are allowed. Only fixed phones (R3_USER=1) are checked.'
        ...
        # RCCONF_39.5.2: 10 цифр с 9; 11 цифр 8x (вторая ≠ 9); 11 цифр 79….
            ...
            if rule_code == "RCCONF_39.5.2":
                error_description = f'Invalid telephone number format in {column_name}. RCCONF_39.5.2: 10 digits starting with 9, or 11 digits 8x (second digit not 9), or 11 digits 79…. All digits only (spaces, dashes, brackets, leading + allowed).'
            else:
