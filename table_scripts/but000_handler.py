"""Основной класс проверки качества данных - ПОЛНОСТЬЮ ИСПРАВЛЕННАЯ ВЕРСИЯ"""
...
class FastDataQualityChecker:
    ...
    def run_quality_checks_fast(self, specific_table: str = None):
        """
        Запускает проверки качества данных - ГЛАВНЫЙ МЕТОД
        """
        print(f"\n" + "="*100)
        ...
        # ДЕБАГ: Покажем таблицы и их колонки
        ...
        for table in ['BUT000', 'KNVP', 'KNB1', 'KNB5', 'DFKKBPTAXNUM']:
            if table in available_tables:
                ...
                if df is not None:
                    ...
                    print(f"Колонки: {list(df.columns)[:10]}{'...' if len(df.columns) > 10 else ''}")
            else:
    ...
    def _process_with_standard_method_forced(self, table_name, df, table_rules, timestamp):
        """Стандартный метод проверки - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    ...
    def _process_with_table_handler_real(self, table_name, df, table_rules, timestamp):
        """Обработка с реальным обработчиком - ПОЛНОСТЬЮ ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        ...
        try:
            ...
            for i, rule in enumerate(table_rules, 1):
                ...
                try:
                    ...
                    # ВАЖНОЕ ИЗМЕНЕНИЕ: Получаем результат ИЗ ОБРАБОТЧИКА
                    error_count = 0
    ...
    def _execute_real_check(self, rule, table_name, df, matched_column, timestamp):
        """ВЫПОЛНЯЕТ РЕАЛЬНУЮ ПРОВЕРКУ правила"""
        rule_code = rule.get("rule_code", "UNKNOWN")
    ...
    def _find_column_with_mapping(self, table_name, available_columns, target_column):
        ...
        # 3. Частичное совпадение (более агрессивное)
        for col in available_columns:
            col_upper = col.upper()
